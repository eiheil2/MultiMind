...
class MemoryManager:
    ...
    def _compact_short_term(self) -> None:
        ...
        self._short_term = [Message(role="system", content=summary)] + self._short_term[self.COMPACT_KEEP :]
    ...
    def assemble_context(
        ...
        # 动态裁剪
        token_est = sum(len(m.content) // 4 for m in context)
        while token_est > max_tokens and len(context) > 5:
            removed = context.pop(0)
            token_est -= len(removed.content) // 4
        return context
    ...
    async def autodream(self, idle_seconds: int = 30) -> None:
        ...
        try:
            ...
            if self._mid_term:
                ...
                if facts:
                    self.add_long_term(MemoryEntry(
    ...
    def list_checkpoints(self) -> list[dict[str, object]]:
        ...
        return [
            {"id": r[0], "commit_hash": r[1], "role": r[2], "timestamp": r[3]}
            for r in rows
        ]
