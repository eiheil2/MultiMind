...
def _load_input_history() -> None:
    ...
    if INPUT_HISTORY_FILE.exists():
        try:
            _input_history = INPUT_HISTORY_FILE.read_text(encoding="utf-8").splitlines()
...
def _save_chat_history() -> None:
    ...
    try:
        ...
        data = {
            ...
            "messages": [{"role": r, "content": c, "time": t} for r, c, t in _chat_history],
        }
...
def _load_last_chat() -> dict[str, Any] | None:
    """加载上次会话的对话历史。"""
    import json
    ...
    if not CHAT_HISTORY_FILE.exists():
        return None
    try:
        data: dict[str, Any] = json.loads(CHAT_HISTORY_FILE.read_text(encoding="utf-8"))
        return data
...
def _print_header() -> None:
    """打印启动横幅 — ASCII 字标 + 霓虹科技感。"""
    banner = (
        f"[{C_ACCENT}]"
        r"  __  __ _   _ _   _____ ___ __  __ ___ _  _ ___  "
        "\n"
        r" |  \/  | | | | | |_   _|_ _|  \/  |_ _| \| |   \ "
        "\n"
        r" | |\/| | |_| | |__ | |  | || |\/| || || .` | |) |"
        "\n"
        r" |_|  |_|\___/|____||_| |___|_|  |_|___|_|\_|___/ "
        "[/]"
    )
    console.print()
    console.print(banner, highlight=False)
    console.print(
        f"[{C_USER}]  ⚡ MultiMind · 多 AI 协作 CLI Agent[/] "
        f"[{C_DIM}]v{APP_VERSION} · Leader / Dispatcher / Executor[/]"
    )
    console.print(f"[{C_DIM}]{'─' * 52}[/]")
    console.print(
        f"[{C_MUTED}]输入消息开始对话，[/]"
        f"[bold {C_ACCENT}]/[/]"
        f"[{C_MUTED}]查看命令，[/]"
...
def _print_footer_hint() -> None:
    """底部快捷键提示。"""
    console.print(f"[{C_DIM}]  / 命令  ·  Tab 补全  ·  ↑↓ 历史  ·  Ctrl+C 退出[/]\n")
...
def _show_last_session_reminder() -> None:
    ...
    console.print(f"[{C_DIM}]上次对话: {saved_at} ({len(messages)} 条消息)[/]")
    if last_user_msg:
        preview = last_user_msg[:60].replace("\n", " ")
        if len(last_user_msg) > 60:
            preview += "..."
        console.print(f"[{C_DIM}]  最后: {preview}[/]")
    console.print(f"[{C_DIM}]  /history 查看本次  ·  /import 导入上次对话[/]\n")
...
def _read_input() -> str:
    ...
    def show_suggestions() -> None:
        ...
        matches = [(cmd, desc, cat) for cmd, desc, cat in SLASH_COMMANDS if cmd.startswith(buf)]
        if not matches:
            draw_prompt()
            return
    ...
    try:
        ...
        async for event in orchestrator.run(user_input, max_rounds=2):
            if event.event_type == OrchestratorEvent.ROLE_START:
                ...
                # 角色标签行
                symbol, color, _ = _TIER_LABELS.get(current_tier, ("●", C_TEXT, current_tier))
                console.print(
                    f"  [{color}]{symbol}[/] "
                ...
                if role_output.strip():
                    ...
                    _chat_history.append(
                        (current_role, role_output.strip(), datetime.now().strftime("%H:%M:%S"))
                    )
...
def _show_status(orchestrator: Orchestrator) -> None:
    ...
    console.print(f"  [{C_DIM}]{'─' * 48}[/]")
    console.print(f"  [{C_MUTED}]拓扑: [/][bold {C_ACCENT}]{orchestrator.topology.describe()}[/]\n")
...
def _show_providers(orchestrator: Orchestrator) -> None:
    ...
    for adapter in orchestrator.registry.all().values():
        ...
        console.print(
            f"  [{C_TEXT}]{name:<20}[/]  [{C_MUTED}]{model:<20}[/]  [{C_DIM}]{channel}[/]"
        )
        if tags:
...
def _show_config() -> None:
    ...
    for label, value, key in config_items:
        console.print(f"  [{C_TEXT}]{label:<14}[/]  [{C_MUTED}]{value}[/]  [{C_DIM}]  ({key})[/]")
...
def _handle_model(args: str, orchestrator: Orchestrator) -> None:
    ...
    if not args:
        ...
        options = [
            (f"{name}  [{adapter.config.model}]", name) for name, adapter in providers.items()
        ]
        selected = _interactive_select(
...
def _handle_prompt(args: str, orchestrator: Orchestrator) -> None:
    ...
    if parts[0] == "reset":
        ...
        from multimind.engine.roles import RoleTier

        deleted = reset_custom_prompt(cast("RoleTier", tier))
        if deleted:
...
def _edit_prompt(tier: str) -> None:
    ...
    # 过滤注释行和空行
    lines = [
        line for line in content.splitlines() if not line.strip().startswith("#") and line.strip()
    ]
    cleaned = "\n".join(lines).strip()
