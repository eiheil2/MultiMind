...
class BrowserAdapter(AIAdapter):
    ...
    async def _ensure_browser(self) -> Any:
        ...
        try:
            ...
            raise AdapterError(
                "Playwright is not installed. Install with: pip install multimind[browser]"
            ) from e
        ...
        try:
            ...
            if self._storage_state and self._storage_state.exists():
                ...
                logger.warning("BrowserAdapter: no storage_state — login required manually")
            ...
            # 无 X server / 浏览器二进制缺失 / TargetClosedError 等——
            # Playwright 抛出的异常类型不一，统一包装为 AdapterError，
            # 否则 ask() 的降级逻辑捕获不到，异常会直接穿透。
            logger.warning("BrowserAdapter: browser launch failed — %s", e)
    ...
    async def ask(
        ...
        **kwargs: object,
        ...
        try:
            ...
            # Playwright 未安装 / 浏览器启动失败 → 降级桩模式
            await asyncio.sleep(0.1)
            yield f"Browser unavailable ({e}). Stub mode output: {prompt[:40]}"
            logger.warning("BrowserAdapter: browser unavailable — %s", e)
            self.record_usage()
            return
        except Exception as e:
            # 兜底：任何未预期异常都不应穿透降级逻辑
            await asyncio.sleep(0.1)
            yield f"Browser error ({e}). Stub mode output: {prompt[:40]}"
            logger.exception("BrowserAdapter: unexpected browser error")
            self.record_usage()
            return
