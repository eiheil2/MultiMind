...
class TestAssembleContext:
    ...
    def test_assemble_context_includes_mid_and_long(self, temp_memory: MemoryManager) -> None:
        """``assemble_context`` merges all three tiers."""

        manager = temp_memory
        manager.add_short_term(Message(role="user", content="short"))
        manager.add_mid_term(
            MemoryEntry(tier=MemoryTier.MID, content="mid note", role="r", tags=["t"])
        )
        manager.add_long_term(
            MemoryEntry(tier=MemoryTier.LONG, content="long fact", role="r", tags=["t"])
        )
    ...
    def test_assemble_context_trims_to_max_tokens(self, temp_memory: MemoryManager) -> None:
    ...
    def test_assemble_context_respects_budget_strictly(self, temp_memory: MemoryManager) -> None:
        """Strict budget: nothing fits in a 1-token budget, so the result is empty.

        The ContextBuilder trims strictly by token budget — there is no
        "safety floor" that keeps messages beyond the budget. Returning an
        empty list here is the honest behaviour: no message can fit into a
        1-token budget, so none is emitted.
        """
...
class TestSummarize:
    ...
    def test_summarize_force_true_with_few_messages(self, temp_memory: MemoryManager) -> None:
    ...
    def test_summarize_persists_mid_term_entry(self, temp_memory: MemoryManager) -> None:
...
class TestMidAndLongTerm:
    ...
    def test_add_mid_term_persists(self, temp_memory: MemoryManager) -> None:
        ...
        manager = temp_memory
        manager.add_mid_term(
            MemoryEntry(tier=MemoryTier.MID, content="stage summary", role="dispatcher")
        )
        ctx = manager.assemble_context("any", max_tokens=100_000)
    ...
    def test_add_long_term_persists(self, temp_memory: MemoryManager) -> None:
        ...
        manager = temp_memory
        manager.add_long_term(
            MemoryEntry(tier=MemoryTier.LONG, content="project archive", role="leader")
        )
        ctx = manager.assemble_context("any", max_tokens=100_000)
    ...
    def test_add_mid_term_writes_to_database(self, temp_memory: MemoryManager) -> None:
        ...
        manager = temp_memory
        manager.add_mid_term(
            MemoryEntry(tier=MemoryTier.MID, content="db-check", role="r", tags=["x"])
        )
...
class TestCheckpoints:
    ...
    def test_list_checkpoints_returns_saved_entries(self, temp_memory: MemoryManager) -> None:
    ...
    def test_list_checkpoints_ordered_newest_first(self, temp_memory: MemoryManager) -> None:
...
class TestAutoDream:
    ...
    @pytest.mark.asyncio
    async def test_autodream_extracts_facts(self, temp_memory: MemoryManager) -> None:
    ...
    @pytest.mark.asyncio
    async def test_autodream_without_facts_is_noop(self, temp_memory: MemoryManager) -> None:
    ...
    @pytest.mark.asyncio
    async def test_autodream_reentrant_guard(self, temp_memory: MemoryManager) -> None:
