...
class TestRole:
    ...
    def test_default_roles_structure(self) -> None:
        """``default_roles`` structure is valid — no hard-coded headcount.

        Instead of pinning an exact count (which breaks on any roster
        change), assert the structural contract: non-empty, unique names,
        valid tiers, and at least one leader.
        """

        roles = default_roles()
        assert roles, "default_roles 不应为空"
        names = [r.name for r in roles]
        assert len(names) == len(set(names)), "角色名应唯一"
        valid_tiers = {"leader", "dispatcher", "executor"}
        assert all(r.tier in valid_tiers for r in roles)
        assert any(r.tier == "leader" for r in roles), "至少需要一个 leader"
...
class TestOrchestrator:
    ...
    async def test_run_yields_events(self, default_providers: ProviderRegistry) -> None:
        ...
        Uses a real subprocess-backed provider (``cat`` echo) so the whole
        pipeline — bus → ContextBuilder → prompt → adapter → events — runs
        for real, offline.
        """
        ...
        registry.register(
            ProviderConfig(name="echo-cli", channel=ChannelType.CLI_REUSE, endpoint="cat")
        )
        roles = [Role(name="指挥官", tier="leader", provider="echo-cli")]
    ...
    async def test_run_posts_to_bus(self, default_providers: ProviderRegistry) -> None:
        ...
        async for _ in orch.run("plan something", max_rounds=2):
            pass
        roles_in_bus = [m.role for m in orch.bus.messages]
        assert "user" in roles_in_bus  # broadcast source is "user" (English)
        assert "指挥官" in roles_in_bus

    @pytest.mark.asyncio
    async def test_run_uses_context_builder(
        self,
        default_providers: ProviderRegistry,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """ContextBuilder 接入验证：角色循环必须经它组装上下文。

        契约：上下文组装统一走 ``ContextBuilder.build``，而不是
        Orchestrator / adapter 各自的硬编码截断。
        """
        ...
        original = orch.context_builder.build

        def spy(
            messages: list[Message], *, query: str = "", max_tokens: int = 8192
        ) -> list[Message]:
            calls.append({"query": query, "max_tokens": max_tokens})
            return original(messages, query=query, max_tokens=max_tokens)

        monkeypatch.setattr(orch.context_builder, "build", spy)
        async for _ in orch.run("hello team", max_rounds=1):
            pass

        assert calls, "ContextBuilder.build 未被调用——上下文组装未接入"
        assert all(c["query"] == "hello team" for c in calls)
        assert all(int(c["max_tokens"]) > 0 for c in calls)

    def test_provider_max_tokens_reads_config(self, default_providers: ProviderRegistry) -> None:
        """``_provider_max_tokens`` 从 provider 配置读取窗口并预留输出空间。"""

        from multimind.core.types import ChannelType, ProviderConfig

        registry = default_providers
        registry.register(ProviderConfig(name="win", channel=ChannelType.LOCAL, max_tokens=10_000))
        orch = Orchestrator(roles=[Role(name="x", tier="leader", provider="win")])
        assert orch._provider_max_tokens(orch.roles[0]) == 7_500

    def test_provider_max_tokens_fallback(self) -> None:
        """provider 未注册时回退保守默认值。"""

        orch = Orchestrator(roles=[Role(name="g", tier="leader", provider="ghost")])
        assert orch._provider_max_tokens(orch.roles[0]) == max(256, int(8192 * 0.75))
